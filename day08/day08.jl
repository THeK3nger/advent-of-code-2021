...
function partOne(input::Array{String,1})
    output = @. split(strip([x[2] for x in split(input, '|')]), ' ')
    sum(count.(x -> length(x) == 2 ||
                        length(x) == 4 ||
                        length(x) == 3 ||
                        length(x) == 7, output))
end
...
function partTwo(input::Array{String,1})
    signal = @. split(strip([x[1] for x in split(input, '|')]), ' ')
    output = @. split(strip([x[2] for x in split(input, '|')]), ' ')
    sum([decodeOutput(output[i], decode(signal[i])) for i = 1:length(input)])
end

sset(x) = Set(collect(x))

function decodeOutput(output, mapping)
    return sum([mapping[sset(output[i+1])] * 10^(3 - i) for i = 0:3])
end

function decode(input)
    one = sset(input[findfirst(x -> length(x) == 2, input)])
    four = sset(input[findfirst(x -> length(x) == 4, input)])
    fourMinusOne = setdiff(four, one)
    ...
    function computeValue(x)
        ...
        elseif length(x) == 5
            ...
            if (four ⊆ sx)
                return 9
            elseif (!(four ⊆ sx) && fourMinusOne ⊆ sx)
                return 6
            else
                return 0
            end
    ...
    return Dict(sset(i) => computeValue(i) for i in input)
end
