...
function partOne(input::Array{String,1})
    ...
    hvsegments = filter(((s, d),) -> any(s .== d), segments)
    countIntersections(hvsegments)
end

#=
Unfortunately, considering only horizontal and vertical lines doesn't give you
the full picture; you need to also consider diagonal lines.  Because of the
limits of the hydrothermal vent mapping system, the lines in your list will only
ever be horizontal, vertical, or a diagonal line at exactly 45 degrees. In other
words:
...
larger - now a total of 12 points.  Consider all of the lines. At how many
...
function parseInput(input::Array{String,1})
    function parsePoint(point)
        tuple(parse.(Int, split(point, ","))...)
    end

    return map(((s, d),) -> (parsePoint(s), parsePoint(d)), split.(input, " -> "))
end

function countIntersections(segments)
    function pointsInSegment(a, b)
        l = max(abs.(a .- b)...)
        δ = @. ifelse(a == b, 0, ifelse(a <= b, 1, -1))
        return [a .+ (n .* δ) for n = 0:l]
    end

    grid = zeros(1000, 1000)
    for s in segments
        points = pointsInSegment(s...)
        for p in points
            grid[(p .+ 1)...] += 1
        end
    end
