...
function partOne(input::Array{String,1})
    (matrix, foldings) = parseInput(input)
    matrix = foldPaper(matrix, foldings[1][2], foldings[1][1])
    return length(matrix)
end

function parseInput(input)
    section = findfirst(x -> x == "", input)
    coordinatesRaw = input[1:section-1]
    foldingsRaw = input[section+1:length(input)]
    coordinatesTuples = map(x -> parse.(Int, split(x, ',')), coordinatesRaw)
    coordinates = Set([CartesianIndex(c[1] + 1, c[2] + 1) for c in coordinatesTuples])
    foldings = [(contains(x, 'y'), parse(Int, split(x, '=')[2]) + 1) for x in foldingsRaw]
    return (coordinates, foldings)
end

function foldPaper(matrix, line, horizontal)
    function foldPoint(ci)
        x, y = ci[1], ci[2]
        y = (horizontal && y > line) ? line * 2 - y : y
        x = (!horizontal && x > line) ? line * 2 - x : x
        return CartesianIndex(x, y)
    end
    return Set([foldPoint(ci) for ci in matrix])
end
...
function partTwo(input::Array{String,1})
    (matrix, foldings) = parseInput(input)
    for f in foldings
        matrix = foldPaper(matrix, f[2], f[1])
    end
    matrixShow(matrix)
    return "(see above)"
end

function matrixShow(matrix)
    C = maximum(matrix)
    M = zeros(Int, C[1], C[2])
