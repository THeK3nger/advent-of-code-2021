...
=#
function partOne(sonar::Array{Int,1})
    return sum([sonar[i] > sonar[i-1] for i = 2:length(sonar)])
end
...
=#
function partTwo(sonar::Array{Int,1})
    window(i) = sonar[i] + sonar[i+1] + sonar[i+2]
    return sum([window(i) > window(i - 1) for i = 2:length(sonar)-2])
end

function main()
    open("input.txt") do f
        sonar = parse.(Int, readlines(f))
        part1result = partOne(sonar)
        part2result = partTwo(sonar)
        printstyled("-- AoC Day 01 --\n", bold = true)
