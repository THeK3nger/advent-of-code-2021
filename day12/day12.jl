...
function partOne(input::Array{String,1})
    ...
    isSmallCave(s) = islowercase(first(s))

    queueSet = [("start", Set())]
    pathCount = 0
    while !isempty(queueSet)
        (current, visitedSmall) = pop!(queueSet)
        if current == "end"
            pathCount += 1
            continue
        end
        newVisited = isSmallCave(current) ?
                     push!(copy(visitedSmall), current) : visitedSmall
        validNeighbors = filter(x -> x ∉ visitedSmall, graph[current])
        push!(queueSet, [(x, newVisited) for x in validNeighbors]...)
    end
    return pathCount
end

function parseGraph(input)
    edges = map(line -> tuple(split(line, '-')...), input)
    graph = Dict()

    function addNode(node, adj)
        if node ∈ keys(graph)
            graph[node] = push!(graph[node], String(adj))
        else
            push!(graph, node => Set([String(adj)]))
        end
    end
...
function partTwo(input::Array{String,1})
    ...
    queueSet = [("start", Set(), false)]
    pathCount = 0
    while !isempty(queueSet)
        # Γ is true if we already visited a small cave twice
        (current, visitedSmall, Γ) = pop!(queueSet)
        if current == "end"
            pathCount += 1
            continue
        end
        if current ∈ visitedSmall && !Γ
            Γ = true
        end
        newVisited = isSmallCave(current) ?
                     push!(copy(visitedSmall), current) : visitedSmall
        # A bit of boolean compression :^)
        validNeighbors = filter(
            x -> (Γ && x ∉ visitedSmall) ||
                (!Γ && x != "start"),
            graph[current])
        push!(queueSet, [(x, newVisited, Γ) for x in validNeighbors]...)
    end
    return pathCount
end
