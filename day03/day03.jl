...
function partOne(input::Array{String,1})
    N = length(input)
    m = hcat(map(x -> Vector{Char}(x), input)...)
    γChars = [count(i -> i == '1', r) > N / 2 ? '1' : '0' for r in eachrow(m)]
    ϵChars = [i == '1' ? '0' : '1' for i in γChars]
    γ = parse(Int, string(γChars...), base = 2)
    ϵ = parse(Int, string(ϵChars...), base = 2)
    return γ * ϵ
end
...
function partTwo(input::Array{String,1})
    B = length(input[1])
    oxygen = hcat(map(x -> Vector{Char}(x), input)...)
    co2 = deepcopy(oxygen)
    # Compute Oxygen
    for b = 1:B
        oxygen = newMatrix((>=), oxygen, b)
        length(oxygen[1, :]) == 1 && break
    end
    oxValue = parse(Int, string(oxygen...), base = 2)
    # Compute CO2
    for b = 1:B
        co2 = newMatrix((<), co2, b)
        length(co2[1, :]) == 1 && break
    end
    co2Value = parse(Int, string(co2...), base = 2)
    return oxValue * co2Value
end

function newMatrix(Criteria, A, b)
    ones = count(x -> x == '1', A[b, :])
    selected = Criteria(ones, length(A[b, :]) - ones) ? '1' : '0'
    AIndexs = findall(x -> x[b] == selected, eachcol(A))
    return A[:, AIndexs]
end
