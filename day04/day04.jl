...
function partOne(input::Array{String,1})
    ...
    game_boards = deepcopy(boards)
    for n in numbers
        # I mark selected numbers in-place with "100".
        game_boards = replace.(game_boards, n => 100)
        for b in game_boards
            if isBingo(b)
                return scoreBoard(b, n)
            end
        end
    end
end

function isBingo(board)::Bool
    return any([all([x == 100 for x in r]) for r in eachrow(board)]) ||
           any([all([x == 100 for x in c]) for c in eachcol(board)])
end

function scoreBoard(board, lastNum)
    # Note that we need to change the selceted numbers (100) to 0 to make the sum.
    return sum(replace(board, 100 => 0)) * lastNum
end
...
function partTwo(input::Array{String,1})
    ...
    for n in numbers
        ...
        for (i, b) in enumerate(game_boards)
            if i ∉ winlist && isBingo(b)
                bingcount += 1
                push!(winlist, i)
                if (bingcount == length(boards))
                    return scoreBoard(b, n)
                end
            end
        end
...
function parse_board_line(nums)
    return getindex.(nums, [i:i+1 for i = 1:3:14]) .|> n -> parse(Int8, n)
end

function parse_input(lines)
    choosen_numbers = split(lines[1], ",") .|> n -> parse(Int8, n)
    board_line_ranges = [i:i+4 for i = 3:6:length(lines)]
