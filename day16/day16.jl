...
function partOne(input::Array{String,1})
    p = BinaryString(input[1])
    bts = String([b for b in p])
    packetOperator(bts)[3]
end
...
function Base.length(x::BinaryString)
    return length(x.hexString) * 4
end

function Base.iterate(x::BinaryString)
    state = (1, 0)
    return iterate(x, state)
end

function Base.iterate(x::BinaryString, state)
    i, j = state
    j += 1
    if j > 4
        i += 1
    ...
    if i > length(x.hexString)
        return nothing
    end
    binaryChunk = bitstring(parse(Int8, x.hexString[i], base = 16))
    return binaryChunk[4+j], (i, j)
end

function parseLiteral(bts, i)
    result = ""
    while bts[i] == '1'
        result = result * bts[i+1:i+4]
        i += 5
    end
    result = result * bts[i+1:i+4]
    i += 4
    return (parse(Int, result, base = 2), i)
end

function packetOperator(bts, i = 1)
    version = parse(Int, bts[i:i+2], base = 2)
    type = parse(Int, bts[i+3:i+5], base = 2)
    i = i + 6
    if type == 4
        result, i = parseLiteral(bts, i)
        return (result, i + 1, version)
    else
        results = []
        if bts[i] == '1'
            subpackages = parse(Int, bts[i+1:i+11], base = 2)
            i += 12
            for _ = 1:subpackages
                result, i, new_version = packetOperator(bts, i)
                version += new_version
                push!(results, result)
            end
        else
            length = parse(Int, bts[i+1:i+15], base = 2)
            i += 16
            while length > 0
                result, new_i, new_version = packetOperator(bts, i)
                version += new_version
                length -= new_i - i
                i = new_i
                push!(results, result)
        ...
        return (Operator[type](results), i, version)
    end
end
